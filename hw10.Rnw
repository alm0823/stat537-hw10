...
The tree based approach is {\it not} invariant to changes in the scale of the predictors. Therefore, when the log10 transformation is done on the predictor variables, the resulting trees, and therefore classification or prediction will also change.

Using the glm approach, rather than the slope coefficient to the log10 transformed variable being the predicted probability (in the binomial response case), the 10 $\times$ the slope coefficient to the log10 transformed variable is the predicted probability of disease.
