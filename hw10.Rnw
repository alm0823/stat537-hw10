...
for (i in 1:100){
  ...
  a <- printcp(tree2)
  ret[i] <- a[min(which(a[, "xerror"] == min(a[, "xerror"]))), "nsplit"]
}
@
<<five.plot,>>=
par(mfrow = c(1, 2), mar = c(5.1, 4.1, 7.1, 2.1))
plotcp(tree1, main = "CV Error vs cp\nfor One Tree\n\n\n")
plot(prop.table(table(ret)), lwd = 10, lend = 1, xaxt = 'n',
     main = "Distribution of the Min-CV Optimal\nNumber of Splits for 100 Trees",
     ylab = "Proportion", xlab = "Number of Splits")
axis(1)
@

\item %6
{\it Based on these results, choose two different sized trees that could be reasonable based on the different CV selections. Report a plot of your two pruned classification trees using the partykit's \verb|plot(as.party(PRUNEDTREENAME))|. Discuss the differences in the two trees.}
